...
void cli(ApartmentList* list)
{
	...
	while (true)
	{
		...
		if (command == "help")
		{
			cout << "add => добавление элемента" << endl;
			cout << "update => обновление элемента" << endl;
			cout << "remove => удаление элемента" << endl;
			cout << "sort => сортировка элементов по одному из двух, доступных для сортировки полей" << endl;
			cout << "olderYear => получить элементы, которые были созданы год назад" << endl;
			cout << "lessThanFloor => получить элементы, у которых этаж меньше либо равны указаному этажу" << endl;
			cout << "lessThanArea => получить элементы, у которых площадь не превышает указаной площади" << endl;
			cout << "searchByStreet => получить элементы, чье название улицы совпадает с указаным" << endl;
			cout << "countByCountRooms => получить количество квартир, с указанным кол-вом комнат" << endl;
		...
		else if (command == "sort")
		{
			...
			int choose;
			cout << "Выберите, по какому полю отсортировать: ";
			cin >> choose;
		...
		else if (command == "countByCountRooms")
		{
			...
			cin.clear();
			cout << "Кол-во квартир с указанным количеством комнат: " << list->getCountByRoomCount(count) << endl << endl;
			system("pause");
			system("cls");
		}
